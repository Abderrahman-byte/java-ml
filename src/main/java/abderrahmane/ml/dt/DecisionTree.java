...
public class DecisionTree implements Predictable {
    ...
    private Map<String, Predictable> leafs;
    ...
    public void build (List<List<String>> table, List<String> classValues) {
        ...
        for (int i = 0; i < featureCount; i++) {
            ...
            for (Entry<String, EntropyEntry> entry: valuesEntropies.entrySet())  {
                entropyValue += Double.valueOf(entry.getValue().getOccurrences()) / rowsCount * entry.getValue().calculate();   
            }
        ...
        for (String value : uniqueDecisionValues) {
            ...
            if (classUniqueValues.size() == 1) {
                leafs.put(value, new Predictable() {
                    @Override
                    public String predict(List<String> values) {
                        return classSubTable.get(0);
                    }
                });
            } else {
